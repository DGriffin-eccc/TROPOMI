...
def download(date, edir, res, levels, times):
    ...
    print("{dir}/single/{d:%Y}/ECMWF_ERA5_RH_VV_{d:%Y%m%d}.nc".format(dir=edir, d=date))
    server.retrieve(
        'reanalysis-era5-pressure-levels',
        {
        'product_type':'reanalysis',
        'format':'netcdf',
        'variable':[ 'relative_humidity','vertical_velocity' ],
        'pressure_level':['1000'],
        ...
        "{dir}/single/{d:%Y}/ECMWF_ERA5_RH_VV_{d:%Y%m%d}.nc".format(dir=edir, d=date))
